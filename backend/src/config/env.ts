...
const envSchema = z.object({
  ...
  // Gates the liability waiver step in onboarding — see legal/private-coaching-waiver.md.
  // Active in production; defaults off here so a fresh/local environment doesn't
  ...
  // Exercise demo illustration generation (admin-triggered, never automatic
  // on a client's workout). Off by default — every generated illustration
  // also requires manual approval before a client can ever see it,
  // regardless of this flag.
  OPENAI_ENABLED: z.string().default("false").transform((val) => val === "true"),
